// src/app/investments/combined-calculator/combined-calculator.component.ts
...
@Component({
  ...
  // You will need to create this SCSS file by copying sip-calculator.component.scss
  styleUrls: ['./combined-calculator.component.scss'] 
})
export class CombinedCalculatorComponent {
  // Input Model
  lumpsumInvestment: number = 50000;
  ...
  analyze(): void {
    if (
        (this.lumpsumInvestment <= 0 && this.monthlyInvestment <= 0) || 
        this.expectedRate <= 0 || 
        this.periodInYears <= 0
    ) {
      alert('Please enter valid positive values for investment amounts, rate, and period.');
      this.isCalculated = false;
      return;
    }

    // Use the new combined calculation service method
    this.yearWiseGrowth = this.calcService.getCombinedGrowth(
  ...
  // Reusing the download functions with updated file names
  downloadExcel(): void {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Combined_Projection');

    const fileName = `Combined_Projection_${this.periodInYears}Y.xlsx`;
  ...
  downloadPdf(): void {
    ...
    if (data) {
      html2canvas(data, { scale: 2 }).then(canvas => {
        ...
        pdf.save(`Combined_Projection_${this.periodInYears}Y.pdf`);
      });
    }
  }
}